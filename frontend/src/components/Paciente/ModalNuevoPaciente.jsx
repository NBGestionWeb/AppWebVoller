...
function ModalNuevoPaciente({ isOpen, onClose, onPacienteGuardado, pacienteAEditar }) {
    ...
    const handleSubmit = async (e) => {
        ...
        const permisoRequerido = pacienteAEditar ? 'edicion_paciente' : 'carga_paciente';
    ...
    // Verificamos si el usuario actual tiene permitido ver el contenido de carga/edición
    const permisoNecesario = pacienteAEditar ? 'edicion_paciente' : 'carga_paciente';
    const autorizado = !loadingPermisos && tienePermiso(permisoNecesario);
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl border border-gray-100 overflow-hidden my-4 sm:my-8 flex flex-col max-h-[90vh]">
                ...
                <div className="bg-terracota-500 px-4 sm:px-6 py-3.5 sm:py-4 flex justify-between items-center text-white shrink-0">
                    ...
                    <button 
                        type="button"
                        onClick={onClose}
                        className="text-white/80 hover:text-white text-2xl font-bold cursor-pointer p-1 leading-none"
                    >
                        &times;
                    </button>
                </div>

                {/* Contenido scrolleable o aviso de denegación */}
                {loadingPermisos ? (
                    <div className="p-8 text-center text-gray-500 text-sm">Verificando permisos...</div>
                ) : !autorizado ? (
                    <div className="p-8 text-center space-y-4">
                        <div className="bg-red-50 border border-red-200 p-4 rounded-xl text-red-700 text-sm font-medium">
                            No cuentas con los permisos autorizados ({pacienteAEditar ? 'Edición de paciente' : 'Carga de paciente'}) para realizar esta operación.
                        </div>
                        <button 
                            type="button"
                            onClick={onClose}
                            className="px-5 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-300 transition-colors cursor-pointer"
                        >
                            Cerrar
                        </button>
                    </div>
